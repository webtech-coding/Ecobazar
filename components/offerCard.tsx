...
const OfferCard:FC<OfferCardProps> =({item, index}):ReactElement=>{
    return(
        <View className="" key={item.id}>
          <Pressable className={`mb-3 h-48 rounded-xl ${index % 2 ? 'flex-row-reverse':'flex-row'} p-1 justify-between items-center flex-1 min-w-full space-x-10`} style={{backgroundColor:item.color}}>
                <View className="p-2">
